        ...
        if handles.scandata.spec.dims == 2
            ...
            switch new_roi.type
                case handles.PROFILE_NAMES{3}
                    ...
                    last = first-1 + n2;
                    page = var2page;
                    image = single(handles.scandata.mcadata(:,d_roi, first:last));
                    delta = sqrt(image);
                ...
                case handles.PROFILE_NAMES{4}
                    ...
                    last = first + n2 * (n3-1);
                    page = var3page;
                    image = handles.scandata.mcadata(:,d_roi, first:n2:last);
                    delta = sqrt(image);
                    if dtcorr
                ...
                case handles.PROFILE_NAMES{5}
                    ...
                    image = handles.scandata.mcadata(:,d_roi,1:last);
                    delta = sqrt(image);
                    if dtcorr
...
%         end
    ...
    case handles.PROFILE_NAMES{6}
        ...
        image = handles.scandata.mcadata(:, d_roi,:);
        delta = sqrt(image);
        if dtcorr
            deadcorr = handles.scandata.dtcorr(d_roi,:);
            dead_delta = handles.scandata.dtdel(d_roi,:);
